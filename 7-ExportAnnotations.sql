...
CREATE TABLE annotations(id serial PRIMARY KEY, id_zone character varying(10), nature character varying (20), cle_origine integer, texte_complet character varying(150), cle_text_part integer, texte character varying(25), x1_y1 character varying(50), x2_y2 character varying(50), x3_y3 character varying(50), x4_y4 character varying(50));	
    ...
    FOR i IN 0..9 LOOP
        FOR j IN 0..9 LOOP
			...
			EXECUTE 'INSERT INTO annotations (id_zone, cle_origine, texte_complet, cle_text_part, texte, x1_y1, x2_y2, x3_y3, x4_y4)
			...
			-- On récupère les textes à annoter et leur bbox en pixels
			EXECUTE 'INSERT INTO annotations (id_zone, cle_origine, texte_complet, cle_text_part, texte) SELECT DISTINCT temporary.zone_name.id_zone as idz, travail.annotationrue.cle_origine, travail.annotationrue.texte_complet, travail.annotationrue.id, travail.annotationrue.text_part
			...
			FOR rec_rue IN EXECUTE 'SELECT DISTINCT id as cle_text_part, cle_origine, ST_ExteriorRing(travail.annotationrue.bbox) AS exterior
        		...
        		FOR s IN 1 .. 4 LOOP
						...
						EXECUTE 'UPDATE annotations SET x'||s||'_y'||s||'= coord_carto_to_image('||x_rue||', '||y_rue||', '||x_min||', '||y_max||') WHERE cle_origine = '''||rec_rue.cle_origine||''' AND cle_text_part = '''||rec_rue.cle_text_part||''';';
        		END LOOP;
   			END LOOP; 
			...
			EXECUTE 'INSERT INTO annotations (id_zone, cle_origine, texte_complet, cle_text_part, texte) SELECT DISTINCT temporary.zone_name.id_zone as idz, travail.annotationriviere.cle_origine, travail.annotationriviere.texte_complet, travail.annotationriviere.id, travail.annotationriviere.text_part
			...
			FOR rec_riv IN EXECUTE 'SELECT DISTINCT cle_origine, id as cle_text_part, ST_ExteriorRing(travail.annotationriviere.bbox) AS exterior
        		...
        		FOR s IN 1 .. 4 LOOP
						...
						EXECUTE 'UPDATE annotations SET x'||s||'_y'||s||'= coord_carto_to_image('||x_riv||', '||y_riv||', '||x_min||', '||y_max||') WHERE cle_origine = '''||rec_riv.cle_origine||''' AND cle_text_part ='||rec_riv.cle_text_part||';';
        		END LOOP;
   			END LOOP; 
